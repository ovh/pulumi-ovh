// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Ovh
{
    public static class GetCloudInstanceGroup
    {
        /// <summary>
        /// Use this data source to retrieve information about an instance group (placement group) in a public cloud project. An instance group is immutable once created: there is no update route and its membership is fixed at instance-creation time.
        /// 
        /// ## Example Usage
        /// 
        /// ```csharp
        /// using System.Collections.Generic;
        /// using System.Linq;
        /// using Pulumi;
        /// using Ovh = Pulumi.Ovh;
        /// 
        /// return await Deployment.RunAsync(() =&gt; 
        /// {
        ///     var @group = Ovh.GetCloudInstanceGroup.Invoke(new()
        ///     {
        ///         ServiceName = "&lt;Public cloud project id&gt;",
        ///         Id = "&lt;instance group id&gt;",
        ///     });
        /// 
        ///     return new Dictionary&lt;string, object?&gt;
        ///     {
        ///         ["groupMemberIds"] = @group.Apply(@group =&gt; .Select(member =&gt; 
        ///         {
        ///             return member.Id;
        ///         }).ToList()),
        ///     };
        /// });
        /// ```
        /// </summary>
        public static Task<GetCloudInstanceGroupResult> InvokeAsync(GetCloudInstanceGroupArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetCloudInstanceGroupResult>("ovh:index/getCloudInstanceGroup:getCloudInstanceGroup", args ?? new GetCloudInstanceGroupArgs(), options.WithDefaults());

        /// <summary>
        /// Use this data source to retrieve information about an instance group (placement group) in a public cloud project. An instance group is immutable once created: there is no update route and its membership is fixed at instance-creation time.
        /// 
        /// ## Example Usage
        /// 
        /// ```csharp
        /// using System.Collections.Generic;
        /// using System.Linq;
        /// using Pulumi;
        /// using Ovh = Pulumi.Ovh;
        /// 
        /// return await Deployment.RunAsync(() =&gt; 
        /// {
        ///     var @group = Ovh.GetCloudInstanceGroup.Invoke(new()
        ///     {
        ///         ServiceName = "&lt;Public cloud project id&gt;",
        ///         Id = "&lt;instance group id&gt;",
        ///     });
        /// 
        ///     return new Dictionary&lt;string, object?&gt;
        ///     {
        ///         ["groupMemberIds"] = @group.Apply(@group =&gt; .Select(member =&gt; 
        ///         {
        ///             return member.Id;
        ///         }).ToList()),
        ///     };
        /// });
        /// ```
        /// </summary>
        public static Output<GetCloudInstanceGroupResult> Invoke(GetCloudInstanceGroupInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetCloudInstanceGroupResult>("ovh:index/getCloudInstanceGroup:getCloudInstanceGroup", args ?? new GetCloudInstanceGroupInvokeArgs(), options.WithDefaults());

        /// <summary>
        /// Use this data source to retrieve information about an instance group (placement group) in a public cloud project. An instance group is immutable once created: there is no update route and its membership is fixed at instance-creation time.
        /// 
        /// ## Example Usage
        /// 
        /// ```csharp
        /// using System.Collections.Generic;
        /// using System.Linq;
        /// using Pulumi;
        /// using Ovh = Pulumi.Ovh;
        /// 
        /// return await Deployment.RunAsync(() =&gt; 
        /// {
        ///     var @group = Ovh.GetCloudInstanceGroup.Invoke(new()
        ///     {
        ///         ServiceName = "&lt;Public cloud project id&gt;",
        ///         Id = "&lt;instance group id&gt;",
        ///     });
        /// 
        ///     return new Dictionary&lt;string, object?&gt;
        ///     {
        ///         ["groupMemberIds"] = @group.Apply(@group =&gt; .Select(member =&gt; 
        ///         {
        ///             return member.Id;
        ///         }).ToList()),
        ///     };
        /// });
        /// ```
        /// </summary>
        public static Output<GetCloudInstanceGroupResult> Invoke(GetCloudInstanceGroupInvokeArgs args, InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<GetCloudInstanceGroupResult>("ovh:index/getCloudInstanceGroup:getCloudInstanceGroup", args ?? new GetCloudInstanceGroupInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetCloudInstanceGroupArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Unique identifier of the instance group to look up.
        /// </summary>
        [Input("id", required: true)]
        public string Id { get; set; } = null!;

        /// <summary>
        /// Service name of the resource representing the id of the cloud project.
        /// </summary>
        [Input("serviceName", required: true)]
        public string ServiceName { get; set; } = null!;

        public GetCloudInstanceGroupArgs()
        {
        }
        public static new GetCloudInstanceGroupArgs Empty => new GetCloudInstanceGroupArgs();
    }

    public sealed class GetCloudInstanceGroupInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Unique identifier of the instance group to look up.
        /// </summary>
        [Input("id", required: true)]
        public Input<string> Id { get; set; } = null!;

        /// <summary>
        /// Service name of the resource representing the id of the cloud project.
        /// </summary>
        [Input("serviceName", required: true)]
        public Input<string> ServiceName { get; set; } = null!;

        public GetCloudInstanceGroupInvokeArgs()
        {
        }
        public static new GetCloudInstanceGroupInvokeArgs Empty => new GetCloudInstanceGroupInvokeArgs();
    }


    [OutputType]
    public sealed class GetCloudInstanceGroupResult
    {
        /// <summary>
        /// Computed hash of the current target specification, used for optimistic concurrency control. Because an instance group has no update route, this value never changes after creation.
        /// </summary>
        public readonly string Checksum;
        /// <summary>
        /// Timestamp at which the instance group was created, in RFC 3339 format.
        /// </summary>
        public readonly string CreatedAt;
        /// <summary>
        /// State of the instance group as observed on the backend:
        /// </summary>
        public readonly Outputs.GetCloudInstanceGroupCurrentStateResult CurrentState;
        /// <summary>
        /// Identifier of the member instance.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// Display name of the instance group as reported by the backend. Fixed for the lifetime of the group, since instance groups cannot be updated.
        /// </summary>
        public readonly string Name;
        /// <summary>
        /// Placement policy currently enforced for the group's members (`AFFINITY`, `ANTI_AFFINITY`). Mirrors the underlying OpenStack/Nova server group policy and is fixed at creation.
        /// </summary>
        public readonly string Policy;
        /// <summary>
        /// Region code.
        /// </summary>
        public readonly string Region;
        /// <summary>
        /// Instance group readiness in the system (`CREATING`, `DELETING`, `ERROR`, `OUT_OF_SYNC`, `READY`). `OUT_OF_SYNC` means the group has drifted from its target specification; a group managed by `ovh.CloudInstanceGroup` is destroyed and recreated on the next `pulumi up` to converge.
        /// </summary>
        public readonly string ResourceStatus;
        public readonly string ServiceName;

        [OutputConstructor]
        private GetCloudInstanceGroupResult(
            string checksum,

            string createdAt,

            Outputs.GetCloudInstanceGroupCurrentStateResult currentState,

            string id,

            string name,

            string policy,

            string region,

            string resourceStatus,

            string serviceName)
        {
            Checksum = checksum;
            CreatedAt = createdAt;
            CurrentState = currentState;
            Id = id;
            Name = name;
            Policy = policy;
            Region = region;
            ResourceStatus = resourceStatus;
            ServiceName = serviceName;
        }
    }
}
